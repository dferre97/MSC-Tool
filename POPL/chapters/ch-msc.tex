...
\begin{definition}[MSC]\label{def:msc}
	...
	\begin{enumerate}
		...
		\end{enumerate}
		\item The \emph{happens-before} relation\footnote{In the literature \etienne{where?}, the relation is also referred to as the \emph{causal ordering} relation and 
...
Note that, according to this definition, we cannot have two messages $m_1$ and $m_2$, both sent by a process $p$ to a process $q$, such that $m_1$ is unmatched and $m_2$ is matched; the unmatched message $m_1$ prevents the receipt of other messages. For instance, the MSC shown in Fig.~\ref{fig:asy_um_ex} is not $\pp$; on the other hand, the MSC in Fig.~\ref{fig:pp_um_ex} is $\pp$ because the two messages are not sent by and addressed to the same process.
...
Note that in a \co-MSC we cannot have two send events $s$ and $s'$ addressed to the same process, such that $s$ is unmatched, $s'$ is matched, and $s \happensbefore s'$. The MSC shown in Fig.~\ref{fig:pp_um_ex} has two send events that are causally related, i.e. $!1 \happensbefore !2$, and the first message $m_1$ is unmatched; however, it is still a $\co$-MSC because the two messages are not addressed to the same process.
