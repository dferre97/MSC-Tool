...
First of all, note that every FIFO $\oneone$ MSC in an asynchronous MSC by definition. Moreover, Fig.~\ref{fig:fully_asy_ex} shows an example of MSC that is asynchronous but not FIFO $\oneone$, hence we have $\ppMSCs \subset \asMSCs$. We saw that in the FIFO $\none$ communicating model any two messages sent to a process $q$ must be received in the same order as they are sent. In the FIFO $\oneone$ communication model we have the same constraint, but only for messages sent by a same process $p$. It is clear to see that FIFO $\none$ communication is a special case of FIFO $\oneone$ communication, and so every FIFO $\onen$ MSC is also a FIFO $\oneone$ MSC. Besides, Fig.~\ref{fig:co_ex}a shows a FIFO $\oneone$ MSC which is not a FIFO $\none$ MSC. In particular, note that $!1 \le !3$, but $!3 \mbrel !1$ since $?3 \procrel ?1$; $\preceq_\msc$ is then cyclic, i.e. not a partial order, and the MSC is not FIFO $\none$. It follows that $\mbMSCs \subset \ppMSCs$. A very similar reasoning can be made for the FIFO $\onen$ communication model, hence $\onenMSCs \subset \ppMSCs$. We now show that each FIFO $\none$ MSC is a causally ordered MSC.

\begin{proposition} \label{prop:mb_is_co}
	Every FIFO $\none$ MSC is a causally ordered MSC.
\end{proposition}
\begin{proof}
Let $\msc$ be a FIFO $\none$ MSC and $\linrel$ a FIFO $\none$ linearization of it. Recall that a linearization has to respect the happens-before partial order over $\msc$, i.e. $\le_\msc \,\subseteq\, \linrel$. Consider any two send events $s$ and $s'$, such that $\lambda(s)=\pqsAct{\plh}{q}$, $\lambda(s')=\pqsAct{\plh}{q}$ and $s \le_\msc s'$. Since $\le_\msc \,\subseteq\, \linrel$, we have that $s \linrel s'$ and, by the definition of FIFO $\none$ linearization, either
...
\begin{figure}[h]
	\begin{center}
		...
		\end{tikzpicture}
		\caption{A causally ordered MSC that is not FIFO $\none$.}
		\label{fig:co_not_none}
	\end{center}
\end{figure}

\begin{proposition} \label{prop:onen_is_co}
	Every FIFO $\onen$ MSC is a causally ordered MSC, i.e. $\onenMSCs \subseteq \coMSCs$.
\end{proposition}
\begin{proof}
...
\davide{Show also that every FIFO $\none$ MSC without unmatched messages is a FIFO $\onen$ MSC.}
...
In the FIFO $\nn$ communication model, any two messages must be received in the same order as they are sent. It is then easy to observe that each FIFO $\nn$ MSC is a FIFO $\onen$ MSC, because each FIFO $\nn$ linearization is also a FIFO $\onen$ linearization. Moreover, Fig.~\ref{fig:onen_nn_rsc_ex}a shows an example of MSC that is FIFO $\onen$ but not FIFO $\nn$, hence we have that $\nnMSCs \subset \onenMSCs$; in particular, note that for messages $m_1$ and $m_4$ we have $!1 \le !4$ and $?4 \procrel ?1$, so there cannot be a $\nn$ linearization, but it is possible to find a $\onen$ linearization such as $!1\;!2\;?2\;!3\;?3\;!4\;?4\;?1$. In the synchronous communication model, every send event is immediately followed by its corresponding receive event. Synchronous communication is then clearly a special case of FIFO $\nn$ communication, and every $\rsc$ MSC is a FIFO $\nn$ MSC because a $\rsc$ linearization is always also a FIFO $\nn$ linearization. Besides, Fig.~\ref{fig:onen_nn_rsc_ex}b shows an example of MSC that is FIFO $\nn$ but not $\rsc$, as a consequence we get $\rscMSCs \subset \nnMSCs$.
...
\begin{figure}[h]
	...
	\begin{subfigure}[t]{0.45\textwidth}\centering
		...
		\end{tikzpicture}
		\caption{A FIFO $\onen$ MSC that is not FIFO $\nn$.}
	\end{subfigure}
	% \hfill
	\begin{subfigure}[t]{0.45\textwidth}\centering
		\begin{tikzpicture}
