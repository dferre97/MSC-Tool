...
As before full proofs can be found in Appendix~\ref{apx:MSO}. 
The first side of the equivalence (Definition \ref{def:n_n_alt} $\Rightarrow$ Definition \ref{def:n_n}) follows from the fact that the order of receive events imposes an order on sends and the fact that a \nn linearization is also a \mb and \onen one.
