...
In Section~\ref{sec:com_models_overview} we introduced 7 different communication models and the corresponding classes of MSCs. Here, we show that all of these classes are MSO-definable, i.e. there is a Monadic Second Order Logic formula that defines them. Sometimes it is straightforward, but for some classes of MSCs we have to introduce alternative definitions (equivalent to those given in Section~\ref{sec:com_models_overview}) that are closer to the logic, in order to prove MSO-definability. These alternative definitions will also be heavily used in the following sections for proofs. We first recall the formal definition of MSO logic over MSCs.
...
As with $\pp$-MSCs, the MSO-definability of $\coMSCs$ follows from Definition~\ref{def:co_msc}, given in Section~\ref{sec:com_models_overview}:
\[
	\coformula = \neg \exists s.\exists s'. \left(
...
The MSO-definability of $\mbMSCs$ follows from Definition~\ref{def:n_one_alt}; in particular, note that $\mbpartial$ is reflexive and transitive by definition, thus we just have to check acyclicity to see if it is a partial order\footnote{A binary relation is acyclic if its transitive closure is antisymmetric.}:
\[
...
the union of the MSO-definable relations $\procrel$, $\lhd$, $\mbrel$, and we may define $x \mbrel y$ as:
\[
x \mbrel y =
\displaystyle
