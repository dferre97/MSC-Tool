...
or \emph{receive} events. A send event $s$ and a receive event $r$ are said to be \emph{matching} if the message sent at $s$ is the one received at $r$. A distributed computation is the \\
\noindent
...
\davide{We should fix vertical space before minipage (lines 66/67), I don't know how to do it.}
%
\noindent in the sense that this MSC does not tell us which one is executed first. Even though events on different processes can be concurrent, this is not always the case. For instance, a send event must always happen
