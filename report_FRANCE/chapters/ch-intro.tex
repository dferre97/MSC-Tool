...
%, whereas any approximation (under or over) of the communication model may hide deadlocks. 
...
In this work we try to clarify and classify some of these communication models.
...
Roughly speaking, a distributed system is composed of processes that can exchange messages. 
Each process executes a sequence of \emph{send} or \emph{receive} \emph{actions}.
A send event $s$ and a receive event $r$ are said to be \emph{matching} if the message sent at $s$ is the one received at $r$. 
%A distributed computation is the parallel execution of all processes. 
\begin{wrapfigure}{r}{0pt}
	...
	\end{tikzpicture}
	\captionof{figure}{An example of MSC. Red arrows denote a causal path.}
	\label{fig:msc_ex}
\end{wrapfigure}
In an MSC, such as the one in Fig.~\ref{fig:msc_ex}, each
...
\begin{figure}[t]
	...
	\centering
%	
	\begin{subfigure}{0.35\textwidth}\raggedleft	
	\begin{tikzpicture}[scale=0.93, every node/.style={transform shape}]
		...
		\draw (2.0,0.85) node[left]{\nnsymb};
		\draw  (0,0) rectangle (2.7,1.8);
		\draw (2.7,1.5) node[left]{\onensymb};
	...
	\end{subfigure} 
	\begin{subfigure}{0.6\textwidth}\raggedleft
	{\small
		\begin{tabular}{| c | c | c|  c| c| } 
			...
			\hline \hline
			$\asy$ & \makecell{unbounded\\STW} & \cmark & \cmark & \cmark \\
			\hline
		...
		\caption{ (Un)decidability results for the synchronizability problems.} %(each 
...
\paragraph{Outline} The report is organized as follows. Section~\ref{sec:MSC} describes the communication models we consider. We also recall the notion of MSC and introduce formal definitions for these models, seen as classes of MSCs.
...
problems based on MSO and on the notion of special treewidth. Related works are discussed all along the report in correspondance to specific notions.
