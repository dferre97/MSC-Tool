...
\davide{This is just a copy-paste of some sections of the concur paper, they need to be changed accordingly. I would also like to add in the beginning some intuitive ideas on (i) what are MSCs (2) why we use them to describe distributed systems computation (3) what we mean by absolute time and executions/linearizations.}
...
\begin{figure}[h]
	...
	\begin{subfigure}[t]{0.5\textwidth}\centering
		...
		\end{tikzpicture}
		\caption{Asynchronous, \pp, not causally ordered, not mailbox, not FIFO $\onen$, not FIFO $\nn$, not $\rsc$.}
	\end{subfigure}
	% \hfill
	\begin{subfigure}[t]{0.5\textwidth}\centering
		\begin{tikzpicture}
...
We find of particular interest to study the relation between the classes of MSCs for all of these communication models. For instance, it is trivial to see that each \pp MSC is also asynchronous (i.e. $\ppMSCs \subseteq \asMSCs$); \pp communication is equivalent to asynchronous communication plus the FIFO constraints, therefore every MSC that respects the \pp communication model is also valid for the asynchronous communication model. It is also quite straightforward to notice that every causally ordered MSC is a \pp MSC. In Section~\ref{} we prove that all these classes of MSCs form a very neat hierarchy, which is graphically shown in Fig.~\ref{fig:msc_hierarchy_full}. 
