...
The latter is an instance of the bounded model-checking problem.
By Fact~\ref{p2p} and Theorem~\ref{th:mailbox_bounded_model_checking}, we obtain decidability.
\end{proof}
...
\davide{Extend Lemma~\ref{lem:prefix-closed} to causal order communication (prefix-closure).}
\davide{Consider also unmatched messages in causal order communication.}
...
% For an MSC $\msc = (\Events,\procrel,\lhd,\lambda)$, we define
% an additional binary relation ${\corel}_\msc \subseteq \Events \times \Events$ that represents a constraint
% under the causal order semantics. In particular, given two events $f_1$ and $f_2$, we say that $f_1 \corel_\msc f_2$ if both the following hold:
% \begin{itemize}\itemsep=0.5ex
...
\right)
\]
The formula $\psi$ says that there cannot be two send events $x$ and $y$, with the same recipient, such that $x \le y$ and the reception of $y$ happens before the reception of $x$. Using $\ppformula$ and $\psi$, the set $\coMSCs$ of causal order MSCs is MSO-definable as $\coMSCs=L(\coformula)$, with $\coformula = \ppformula \,\wedge\, \psi$.
...
\end{proposition}
\begin{proof}
Follows from Corollary~\ref{cor:weak-sync-lcpdl} and from the fact that the set $\coMSCs$ of causal order MSCs is MSO-definable, as shown in the proof of Theorem~\ref{th:co_bounded_model_checking}.
\end{proof}
...
\end{proposition}
\begin{proof}
Directly follows from Proposition~\ref{prop:weak-logic-bounded} and from the fact that the set $\coMSCs$ of causal order MSCs is MSO-definable, as shown in the proof of Theorem~\ref{th:co_bounded_model_checking}.
\end{proof}
...
\end{theorem}
\begin{proof}
By Proposition~\ref{prop:co-weak-k-sync-mso} and Proposition~\ref{prop:kweakstw} we have that the class of causally ordered \emph{k}-synchronous MSCs is MSO-definable and STW-bounded. We can use Theorem~\ref{thm:sync_co} to end the proof.
\end{proof}
