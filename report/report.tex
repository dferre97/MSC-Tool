...
Intuitively, an MSC is causally ordered if all the messages sent to the same process are received in an order which is consistent with the causal ordering of the corresponding send events. Below the formal definition, which also considers unmnatched messages.
...
Let $\msc = (\Events, \procrel, \lhd, \lambda) \in \coMSCs$ and let $\msc_0 =
(\Events_0, \procrel_0, \lhd_0, \lambda_0)$ be a prefix of $\msc$. By contradiction, suppose that $\msc_0$ is not a	causally ordered MSC. Hence, there must be two distinct $s,s' \in \Events_0$ such that $\lambda(s)=\pqsAct{\plh}{q}$, $\lambda(s')=\pqsAct{\plh}{q}$, $s \le_{\msc_0} s'$ and either
\begin{enumerate*}[label={(\roman*)}]
...
\begin{itemize}
	\item \emph{Peer-to-peer}: in the peer-to-peer (\pp) architecture, there is a channel between each pair $(P_1,P_2)$ of machines, such that $P_1$ can send messages to $P_2$. Note that, given two machines $P_1$ and $P_2$ that can exchange messages in both directions, we have two channels: one for the messages sent by $P_1$ to $P_2$, and the other for the messages sent by $P_2$ to $P_1$. When talking about \pp we suppose that channels operate in FIFO mode (i.e. as queues), but we will also explore other variants, as discussed later.
	\item \emph{Mailbox}: in the mailbox architecture, each machine merges all of its incoming messages (from any other machine) into a unique queue. In other words, each machine $P_i$ has a single incoming FIFO channel, which is shared by the other machines that send messages to $P_i$.
\end{itemize}
For convenience, we will refer to a system that uses the \pp architecture simply as a \pp system (same for mailbox and other variants).

\smallskip

\noindent We will also consider some variants of these communication architectures:
\begin{itemize}
	\item \emph{Causal ordering}: a \pp system implements causal ordering if it ensures that all the messages sent to the same machine are received in an order which is consistent with the causal ordering of the send events. Channels still operate in FIFO mode, but the delivery of some messages might be delayed to enforce causal ordering.
\end{itemize}
