...
Following the characterization given in \cite[Theorem 4.4]{DBLP:journals/dc/Charron-BostMT96}, we give an alternative but equivalent definition of $\rsc$ MSC, which is based on a property of the conflict graph.

\begin{definition} [$\rsc$ alternative]\label{def:rsc_alt}
	An MSC $\msc = (\Events,\procrel,\lhd,\lambda)$ is a \emph{RSC MSC} if and only if it has no unmatched send events and if in $CG(\msc)$ there is no cycle, of $\text{length} \ge 2$, whose edges are all SR.
\end{definition}
...
Following Definition~\ref{def:rsc_alt}, an MSC $\msc$ is a $\rsc$ MSC if and only if it satisfies the LCPDL formula
\[
\Phi_{\rsc} = \neg \Loop{\xrightarrow{SR} \cdot (\xrightarrow{SR})^\ast}
\]
Recall that every LCPDL-definable property is also MSO-definable. In particular, this is one way to write the same property using MSO:
\[
\Phi_{\rsc} = \neg \exists s_1.\exists s_2. s_1 \rightarrow_{SR} s_2 \;\wedge\; s_2 \rightarrow_{SR}^\ast s_1
\]
\noindent where $\rightarrow_{SR}$ is defined as
\[
s_1 \rightarrow_{SR} s_2 = 
\bigvee_{\substack{e in \sAct}(\lambda(s_1) = e)} \;\wedge\; 
\exists s_2.\exists r_2. (s_1 \procrel^+ r_2 \;\wedge\; s_2 \lhd r_2)
\]
